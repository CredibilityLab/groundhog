...
#Set 0 - various forms of calling packages to be loaded
  ...
  #Object with pkg nmae no quotes
    library('groundhog')
...
#1) Loading but not attaching dependencies (verify one can run lmer() without lme4::)
    ...
    groundhog.library(pkgs,'2021-09-01')   #Date altered by hand to avoid error generated by problematic version of Rcpp
...
#5) Test source version
    library('groundhog')
    groundhog.library('pwr','2018-08-01',force.source = TRUE)
    
#6) Test for re-install
    library('groundhog')
    groundhog.library('pwr','2018-08-01',force.source = TRUE,force.install = TRUE)
...
#10) Test on standalone unix machine which can produce errors connecting to groundhogr (due to agent)
#    and loading without specifying a mirror
     groundhog:::load.cran.toc(TRUE)  #check if it can get files from groundhogr
    
    
#11) Tolerate older R version
     #Run in R-3.6.x
     groundhog.library('pwr', "2021-08-15",tolerate.R.version = '3.6.3')
