...
install.snowball=function(pkg,date, include.suggests, force.install=FALSE,force.source=FALSE,plot.console=TRUE,quiet.install=TRUE)
{
  ...
  for (k in 1:nrow(snowball))
  {
    ...
    if (!snowball[k,'installed'])
    {
      ...
      if (from.k=="MRAN") {
        #Try MRAN 1
        install.packages(pkg.k,lib=lib.k, type='binary', repos=paste0('http://mran.microsoft.com/snapshot/',mran.date.k,'/'),dependencies = FALSE,quiet=quiet.install, INSTALL_opts = "--no-staged-install")
        #Try MRAN 2: If still not installed, try day after (some MRAN days are missing)
        if (!is.pkg_vrs.installed(pkg.k,vrs.k)) install.packages(pkg.k,lib=lib.k, type='binary', repos=paste0('http://mran.microsoft.com/snapshot/',mran.date.k+1,'/'),dependencies = FALSE,quiet=quiet.install, INSTALL_opts = "--no-staged-install")
        #Try MRAN 3: If still not installed, try 2 days earlier
        if (!is.pkg_vrs.installed(pkg.k,vrs.k))  install.packages(pkg.k,lib=lib.k, type='binary', repos=paste0('http://mran.microsoft.com/snapshot/',mran.date.k-2,'/'),dependencies = FALSE,quiet=quiet.install, INSTALL_opts = "--no-staged-install")
      } #end MRAN
