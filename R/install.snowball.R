...
install.snowball <- function(pkg, date, include.suggests, force.install = FALSE,
  ...
  for (k in seq_len(nrow(snowball))) {
    ...
    if (!snowball[k, "installed"]) {
      ...
      if (from.k == "CRAN") {
        install.packages(pkg.k, dependencies = FALSE, lib = lib.k, repos = "https://cloud.r-project.org", type = "both", quiet = quiet.install, INSTALL_opts = "--no-staged-install", keep_outputs = file.path(get.groundhog.folder(), "logs/"))
      ...
      if (from.k == "MRAN") {
        # Try MRAN 1
        install.packages(pkg.k, lib = lib.k, type = "binary", repos = paste0("https://mran.microsoft.com/snapshot/", mran.date.k, "/"), dependencies = FALSE, quiet = quiet.install, INSTALL_opts = "--no-staged-install", keep_outputs = file.path(get.groundhog.folder(), "logs/"))
        ...
        if (!is.pkg_vrs.installed(pkg.k, vrs.k)) {
          install.packages(pkg.k, lib = lib.k, type = "binary", repos = paste0("https://mran.microsoft.com/snapshot/", mran.date.k + 1, "/"), dependencies = FALSE, quiet = quiet.install, INSTALL_opts = "--no-staged-install", keep_outputs = file.path(get.groundhog.folder(), "logs/"))
          message1("Attempt #2 to install binary from from MRAN")
        }
        # Try MRAN 3: If still not installed, try 2 days earlier
        if (!is.pkg_vrs.installed(pkg.k, vrs.k)) {
          install.packages(pkg.k, lib = lib.k, type = "binary", repos = paste0("https://mran.microsoft.com/snapshot/", mran.date.k - 2, "/"), dependencies = FALSE, quiet = quiet.install, INSTALL_opts = "--no-staged-install", keep_outputs = file.path(get.groundhog.folder(), "logs/"))
