    ...
    ignore.deps, force.source , force.install , tolerate.R.version )
      ...
      #2 Validate pkg 
        validate.pkg_vrs(pkg,date,ignore.deps)
            ...
            #See validate.pkg_vrs.R
            #Check not using groundhog, not loading another version of the same package, 
            #and quickly attaches if already loaded
    
      #3 Validate R version being used for date  
          validate_R(pkg, date, tolerate.R.version)
          
            #See validate_R.R  
            #Gives warnings for mismatch date and R
